// FP32 TT GEMM: C = A^T @ B^T where A is MxK, B is KxN
...
void fp32_gemm_tt(float* __restrict__ C, const float* __restrict__ A,
    ...
    // A: KxM — need transpose
    const float* A_ptr = A;
    int A_m_off = tm * BS;
    // B: NxK — same as NT (buffer_load)
    const float* B_base = B + tn * BS * K;
    i32x4 b_sr = make_srsrc(B_base, BS*K*4, K*4);
    ...
    for(int idx=threadIdx.x;idx<TOTAL;idx+=NT){int r=idx/LPR,c4=(idx%LPR)*4;int bo2=(r*K+c4)*4;
        ...
        *(float4*)&Bs[r][c4]=*(float4*)&rb;}}
    load_B_nn_fp32(As, A_ptr + A_m_off, M, 0);  // transpose A, not B
    __syncthreads();
    ...
    for(int kt=0; kt<K/KS-1; kt++) {
        ...
        __syncthreads();
        if(idx<BS*KS/4){int r=idx/(KS/4),c4=(idx%(KS/4))*4;*(float4*)&Bs[r][c4]=ab;}
        load_B_nn_fp32(As, A_ptr + A_m_off, M, (kt+1)*KS);  // transpose next A
        __syncthreads();
    }
...
#define HK_MODULE_NAME hk_fp32_tt
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        fp32_gemm_tt<<<dim3((Nsz/BS)*(Msz/BS)),dim3(NT),0,(hipStream_t)0>>>(
    ...
    }, "FP32 TT GEMM: C = A^T @ B^T (in-kernel transpose)");
}
