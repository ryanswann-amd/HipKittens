// FP32 TN GEMM via fused transpose+NT: C = A^T @ B where B is KxN
...
#define HK_MODULE_NAME hk_fp32_tn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        int Ksz=sa[0].cast<int>(), Msz=sa[1].cast<int>(), Nsz=sb[1].cast<int>(); // A=KxM, B=KxN
        ...
        size_t need_a=(size_t)Msz*Ksz*sizeof(float), need_b=(size_t)Nsz*Ksz*sizeof(float), need=need_a+need_b;
        if(need>ws_sz){if(ws)hipFree(ws);hipMalloc(&ws,need);ws_sz=need;}
        float* ws_a=ws; float* ws_b=ws+Msz*Ksz;
        dim3 tg_a((Msz+TILE_DIM-1)/TILE_DIM,(Ksz+TILE_DIM-1)/TILE_DIM);
        dim3 tg_b((Nsz+TILE_DIM-1)/TILE_DIM,(Ksz+TILE_DIM-1)/TILE_DIM);
        transpose_f32<<<tg_a,dim3(TILE_DIM,BLOCK_ROWS)>>>(ws_a,(const float*)a_ptr,Ksz,Msz);
        transpose_f32<<<tg_b,dim3(TILE_DIM,BLOCK_ROWS)>>>(ws_b,(const float*)b_ptr,Ksz,Nsz);
        fp32_gemm_nt<<<dim3((Nsz/BS)*(Msz/BS)),dim3(NT_THR),0,(hipStream_t)0>>>((float*)c_ptr,ws_a,ws_b,Msz,Nsz,Ksz);
    }, "FP32 TN GEMM via transpose+NT: C = A^T @ B");
}
