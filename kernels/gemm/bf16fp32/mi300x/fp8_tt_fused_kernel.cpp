// FP8 TT GEMM via fused transpose+NT: C = A^T @ B^T where B is KxN
//
...
#define HK_MODULE_NAME hk_fp8_tt_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        int Nsz = sb[0].cast<int>();  // B is NxK
        ...
        // Static workspace for transposed A (MxK)
        static char* workspace = nullptr;
        static size_t workspace_size = 0;
        size_t needed = (size_t)Msz * Ksz;
        ...
        transpose_fp8<<<tg, tb>>>((char*)workspace, (const char*)a_ptr, Ksz, Msz);

        // Phase 2: FP8 NT GEMM with transposed A (MxK) and B (NxK)
        fp8_gemm_nt<<<dim3((Nsz/BS)*(Msz/BS)), dim3(NT_THR), 0, (hipStream_t)0>>>(
            (float*)c_ptr, (const char*)workspace, (const char*)b_ptr, Msz, Nsz, Ksz);
    }, "FP8 TT GEMM via transpose+NT: C = A^T @ B^T");
}
