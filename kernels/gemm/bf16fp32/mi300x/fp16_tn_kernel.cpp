// FP16 TN GEMM: C = A^T @ B where A is KxM (row-major), B is KxN (row-major)
//
...
void micro_tk(const micro_globals g) {
    ...
    const half* A_base = (const half*)&g.a[{0, 0, 0, 0}];
    const half* B_base = (const half*)&g.b[{0, 0, 0, 0}];
...
#define HK_MODULE_NAME hk_fp16_tn
#endif
