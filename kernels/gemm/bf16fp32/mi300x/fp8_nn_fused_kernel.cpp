// FP8 NN GEMM via fused transpose+NT: C = A @ B where B is KxN
//
...
#define HK_MODULE_NAME hk_fp8_nn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        // Static workspace for transposed B (NxK)
        static char* workspace = nullptr;
        static size_t workspace_size = 0;
        size_t needed = (size_t)Nsz * Ksz;  // 1 byte per FP8 element
        ...
        dim3 tg((Nsz + TILE_DIM - 1) / TILE_DIM, (Ksz + TILE_DIM - 1) / TILE_DIM);
        dim3 tb(TILE_DIM, BLOCK_ROWS);
        transpose_fp8<<<tg, tb>>>((char*)workspace, (const char*)b_ptr, Ksz, Nsz);

        // Phase 2: FP8 NT GEMM with transposed B (now NxK)
        fp8_gemm_nt<<<dim3((Nsz/BS)*(Msz/BS)), dim3(NT_THR), 0, (hipStream_t)0>>>(
            (float*)c_ptr, (const char*)a_ptr, (const char*)workspace, Msz, Nsz, Ksz);
    }, "FP8 NN GEMM via transpose+NT: C = A @ B");
}
