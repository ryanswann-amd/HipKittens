// FP8 TT GEMM: C = A^T @ B^T where A is KxM, B is NxK
//
// A needs in-kernel transpose (KxM → MxK in shared).
// B is already NxK (same layout as NT). No B transpose needed.
...
constexpr int AS_PAD = KS + 4;  // LDS padding for bank conflict elimination
...
// Transpose load: rows×cols global (stride=cols) → shared[col][row] (transposed)
template<int STRIDE>
...
void fp8_gemm_tt(float* __restrict__ C, const char* __restrict__ A,
                 const char* __restrict__ B, int M, int N, int K) {
    __shared__ char As[BS][AS_PAD], Bs[BS][KS];  // As padded (A transpose), Bs unpadded (NT layout)
    ...
    // A: KxM — transpose A(KxM) to As[m][k]
    int A_m_offset = tm * BS;
    // B: NxK — same as NT, load directly
    const char* Bb = B + tn*BS*K;
    i32x4 b_sr = make_srsrc(Bb, BS*K, K);
    const int total_b_loads = (BS * KS) / 16;

    // Prologue
    load_transpose_fp8<AS_PAD>(As, A, M, 0, A_m_offset);
    for(int i=threadIdx.x; i<total_b_loads; i+=NT) {
    ...
    for(int kt=0;kt<num_tiles-1;kt++){
        // Prefetch B via buffer_load, prefetch A via transpose
        float4 b_buf;
        {int i=threadIdx.x; if(i<total_b_loads){
            int r=i/(KS/16),c16=(i%(KS/16))*16;
        ...
        load_transpose_fp8<AS_PAD>(As, A, M, (kt+1)*KS, A_m_offset);
        asm volatile("s_waitcnt lgkmcnt(0)");
        __builtin_amdgcn_s_barrier();
    }
...
#define HK_MODULE_NAME hk_fp8_tt
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        int Ksz=sa[0].cast<int>(), Msz=sa[1].cast<int>(), Nsz=sb[0].cast<int>();
        fp8_gemm_tt<<<dim3((Nsz/BS)*(Msz/BS)),dim3(NT),0,(hipStream_t)0>>>(
    ...
    }, "FP8 TT GEMM: C = A^T @ B^T (in-kernel A transpose)");
}
