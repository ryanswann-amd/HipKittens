// BF16 TN GEMM via fused transpose+NT: C = A^T @ B where A is KxM, B is KxN
//
...
#define HK_MODULE_NAME hk_tn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        static bf16* ws_a = nullptr;
        static bf16* ws_b = nullptr;
        static size_t ws_a_size = 0, ws_b_size = 0;
        size_t need_a = (size_t)Msz * Ksz * sizeof(bf16);
        size_t need_b = (size_t)Nsz * Ksz * sizeof(bf16);
        if (need_a > ws_a_size) {
    ...
    }, "BF16 TN GEMM via transpose+NT: C = A^T @ B");
}
