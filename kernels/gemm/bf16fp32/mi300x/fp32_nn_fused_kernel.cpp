// FP32 NN GEMM via fused transpose+NT: C = A @ B where B is KxN
...
#define HK_MODULE_NAME hk_fp32_nn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        size_t need=(size_t)Nsz*Ksz*sizeof(float);
        if(need>ws_sz){if(ws)hipFree(ws);hipMalloc(&ws,need);ws_sz=need;}
        dim3 tg((Nsz+TILE_DIM-1)/TILE_DIM,(Ksz+TILE_DIM-1)/TILE_DIM);
        transpose_f32<<<tg,dim3(TILE_DIM,BLOCK_ROWS)>>>(ws,(const float*)b_ptr,Ksz,Nsz);
        fp32_gemm_nt<<<dim3((Nsz/BS)*(Msz/BS)),dim3(NT_THR),0,(hipStream_t)0>>>((float*)c_ptr,(const float*)a_ptr,ws,Msz,Nsz,Ksz);
    }, "FP32 NN GEMM via transpose+NT: C = A @ B");
}
