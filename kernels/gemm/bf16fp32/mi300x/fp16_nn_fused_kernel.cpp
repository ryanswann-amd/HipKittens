// FP16 NN GEMM via fused transpose+NT: C = A @ B where B is KxN
//
// For large sizes where the in-kernel transpose at 256x256 spills,
// this kernel uses a two-phase approach:
//   Phase 1: Lightweight transpose kernel B_KxN → B_NxK (bandwidth-limited, ~50µs for 8K)
//   Phase 2: Full-speed NT kernel (580 TF at 256x256, 0 spills)
//
// The temporary NxK buffer is allocated by the caller. The combined
// overhead is ~2-3% vs pure NT, giving ~560 TF for NN at large sizes.
//
// This mirrors hipBLASLt's approach of using workspace buffers for
// certain transpose configurations.
...
void micro_tk(const micro_globals g) {
    ...
    for (int tile = 0; tile < num_tiles - 1; ++tile) {
        ...
        float4 b_buffer_next[BUFFER_SIZE * sizeof(half) / sizeof(float4)];
...
#define HK_MODULE_NAME hk_fp16_nn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        int Msz = sa[0].cast<int>();
        int Ksz = sa[1].cast<int>();
        ...
        // Static workspace cache — avoids hipMalloc/hipFree per call
        static half* workspace = nullptr;
        static size_t workspace_size = 0;
        size_t needed = (size_t)Nsz * Ksz * sizeof(half);
        ...
        transpose_kernel<<<trans_grid, trans_block>>>(
            workspace, (const half*)b_ptr, Ksz, Nsz);

        // Phase 2: NT GEMM with transposed B (now NxK)
        auto ga = kittens::make_gl<_gl_A>(a_ptr, 1, 1, Msz, Ksz);
        auto gb = kittens::make_gl<_gl_B>((uint64_t)workspace, 1, 1, Nsz, Ksz);
    ...
    }, "FP16 NN GEMM via transpose+NT: C = A @ B");
}
