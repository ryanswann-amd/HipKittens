// FP32 TN GEMM: C = A^T @ B where A is MxK, B is KxN
...
void fp32_gemm_tn(float* __restrict__ C, const float* __restrict__ A,
    ...
    load_B_nn_fp32(As, A_ptr + A_m_off, M, 0);
    ...
    for(int kt=0; kt<K/KS-1; kt++) {
        ...
        __syncthreads();
        load_B_nn_fp32(As, A_ptr + A_m_off, M, (kt+1)*KS);
...
#define HK_MODULE_NAME hk_fp32_tn
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        fp32_gemm_tn<<<dim3((Nsz/BS)*(Msz/BS)),dim3(NT),0,(hipStream_t)0>>>(
    ...
    }, "FP32 TN GEMM: C = A^T @ B (in-kernel transpose)");
}
