// FP16 TN GEMM via fused transpose+NT: C = A^T @ B where A is KxM, B is KxN
//
// Transpose BOTH: A_KxM → A_MxK, B_KxN → B_NxK, then NT kernel.
// Two transpose kernels + one NT kernel. Overhead ~4-6% at large sizes.
...
#define HK_MODULE_NAME hk_fp16_tn_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        // Workspace for both transposed A and transposed B
        static half* ws_a = nullptr;
        static half* ws_b = nullptr;
        static size_t ws_a_size = 0, ws_b_size = 0;
        size_t need_a = (size_t)Msz * Ksz * sizeof(half);
        size_t need_b = (size_t)Nsz * Ksz * sizeof(half);
        if (need_a > ws_a_size) {
            if (ws_a) hipFree(ws_a);
            hipMalloc(&ws_a, need_a);
            ws_a_size = need_a;
        }
        if (need_b > ws_b_size) {
            if (ws_b) hipFree(ws_b);
            hipMalloc(&ws_b, need_b);
            ws_b_size = need_b;
        }
        ...
        transpose_kernel<<<grid_a, trans_block>>>(
            ws_a, (const half*)a_ptr, Ksz, Msz);

        // Phase 1b: Transpose B from KxN to NxK
        dim3 grid_b((Nsz + TILE_DIM - 1) / TILE_DIM, (Ksz + TILE_DIM - 1) / TILE_DIM);
        transpose_kernel<<<grid_b, trans_block>>>(
            ws_b, (const half*)b_ptr, Ksz, Nsz);

        // Phase 2: NT GEMM with A_MxK × B_NxK
        auto ga = kittens::make_gl<_gl_A>((uint64_t)ws_a, 1, 1, Msz, Ksz);
        auto gb = kittens::make_gl<_gl_B>((uint64_t)ws_b, 1, 1, Nsz, Ksz);
    ...
    }, "FP16 TN GEMM via transpose+NT: C = A^T @ B");
}
