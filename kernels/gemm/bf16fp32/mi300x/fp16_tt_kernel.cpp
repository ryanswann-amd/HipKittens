// FP16 TT GEMM: C = A^T @ B^T where A is KxM (row-major), B is NxK (row-major)
//
    ...
    int k_tile, int m_tile)
{
    ...
    for (int batch = threadIdx.x; batch < TOTAL; batch += N_THR) {
        ...
        // 8 quad writes: As[m+i, k:k+3] as 64-bit (4 half)
        #pragma unroll
...
#define HK_MODULE_NAME hk_fp16_tt
#endif
