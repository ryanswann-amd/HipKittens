// FP16 TT GEMM via fused transpose+NT: C = A^T @ B^T where A is KxM, B is NxK
//
...
#define HK_MODULE_NAME hk_fp16_tt_fused
#endif
PYBIND11_MODULE(HK_MODULE_NAME, m) {
    m.def("dispatch", [](pybind11::object A, pybind11::object B, pybind11::object C) {
        ...
        size_t needed = (size_t)Msz * Ksz * sizeof(half);
        ...
        transpose_kernel<<<trans_grid, trans_block>>>(
            workspace, (const half*)a_ptr, Ksz, Msz);
    ...
    }, "FP16 TT GEMM via transpose+NT: C = A^T @ B^T");
}
